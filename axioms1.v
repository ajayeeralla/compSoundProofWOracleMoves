...
Axiom FUNCApp_elt :   forall (p   :nat) {m} (ml1 ml2:mylist m), (ml1~ml2) ->  (ml1 ++ [getelt_at_pos  p ml1 ] ) ~ (ml2 ++ [  getelt_at_pos p ml2]).
...
(** * Authenticated version of DH Protocol (STS Protocol) *)
(** * Digital Signatures*)
(** ** Assumptions *)
...
(** Existential unforgeability under adaptively chosen message attacks (UF-CMA secure) *)
  ...
  Fixpoint unforgb  (j:nat) (k:nat) {n: nat} (ml: ilist message (n)) (t u :message) : Bool :=
    match j, ml with
      |  0 , _  => FAlse
      |  S _, []  => FAlse             
      | S j', h::tl => if_then_else_B  (EQ_M t h) (ver (pk (N k)) h u) (unforgb j' k tl t u)        
    end.    
 
Axiom UFCMA : forall (n l :nat)(ml: ilist message l) (t u: message), (clos_mylist [ msg t; msg u] = true) /\ (onlyin_pkrsk_mylist n [msg t; msg u] = true) /\ (skn_in_sign_mylist n [msg t; msg u] = true) /\ (l = length(list_skn_in_sign n (app ( subtrmls_msg t) ( subtrmls_msg u))))   ->  (ver (pk (N n)) t u) ## (unforgb l n ml t u).
...
(** Computational Hiding and Unconditional Binding *)

Definition pkn (n:nat) := (pk (N n)).

Definition skn (n:nat) := (sk (N n)).
(*


Axiom comphid: forall (n n0 n1 m:nat) ( t t': message) (l1 l2 :mylist n), (Fresh [ n0 ; n1] (l1 ++ l2)) = true -> (l1 ~ l2) -> (l1 ++ [msg (commit (pkn m) t (rr (N n0)))]) ~ (l2 ++ [msg (commit (pkn m) t' (rr (N n1)))]).

Axiom ucondbind: forall (n m:nat) (pk r r' t t':message) (l:mylist n), (l ++ [bol (if_then_else_B (EQ_M t t')&(EQ_M r r') FAlse (EQ_M (commit (pkn m) t r) (commit (pkn m) t' r')))]) #### (l ++ [bol FAlse]). *)



(** **Blind Singnatures *)
(** ** Assumptions *)
(** Correctness, Unforgeability, and Blindness *)

Axiom blindness: forall (n n' n'' m:nat)  (t m0 m1: message) (l: mylist m), (Fresh [n'; n''] (l++[msg t; msg m0; msg m1]) = true) ->
                                                                         (l ++ [msg m0; msg m1; msg (blind m0 (r n')); msg (blind m1 (r n'')); msg (if_then_else_M (bacc (pkn n) (blind m0 (r n')) (f [(blind m0 (r n')); (skn n); (pkn n)])) (unblind (f [(blind m0 (r n')); (skn n); (pkn n)]) (r n')) t); msg (if_then_else_M (bacc (pkn n) (blind m1 (r n'')) (f [(blind m1 (r n'')); (skn n); (pkn n)])) (unblind (f [(blind m1 (r n'')); (skn n); (pkn n)]) (r n'')) t)]) ~
(l ++ [msg m0; msg m1; msg (blind m1 (r n'')); msg (blind m0 (r n')); msg (if_then_else_M (bacc (pkn n) (blind m1 (r n'')) (f [(blind m1 (r n'')); (skn n); (pkn n)])) (unblind (f [(blind m1 (r n'')); (skn n); (pkn n)]) (r n'')) t); msg (if_then_else_M (bacc (pkn n) (blind m0 (r n')) (f [(blind m0 (r n')); (skn n); (pkn n)])) (unblind (f [(blind m0 (r n')); (skn n); (pkn n)]) (r n')) t)]).                                                                                                                                                                                                                                                                                                                                                                                                                                           
...
Axiom corr_comit: forall (n:nat) (m r: message), (open (pkn n) (commit (pkn n) m r) r) # m.

Axiom corr_blind: forall (m r: message), (unblind (blind m r) r) # m.

Axiom ret_sign: forall (n:nat) (m r:message), (unblind (bsign (skn n) (blind m r)) r) # (bsign (skn n) m).

Axiom bvercorr:  forall (n:nat) (m r:message), (bver (pkn n) (blind m r) (bsign (skn n) (blind m r))) ## TRue.

Axiom bacc_corr: forall (n:nat) (m r :message), (bacc (pkn n) (blind m r) (bsign (skn n) (blind m r))) ## TRue.
