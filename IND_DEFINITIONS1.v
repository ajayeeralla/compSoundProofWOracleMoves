...
(***********if b then t = if b then t else o********************************)
...
(**********************G(_)= pi1 (ggen(_)) and g(_)= pi2(ggen(_))*******)
Definition G (n: nat) := (pi1 (ggen (N n))).
Definition g( n:nat) := (pi2 (ggen (N n))).



(**********************pk(x) = pi1 (k(x)) and sk(x) = pi2 (k(x))************)
...
(*********************list message to mylist (length l )*********************)
...
(**************************Check for given names in terms: message ; Bool***********)
(*********************************************************************************)
...
(********************exp on given elts at positions***********************************)
...
(******************* (EQ m1 m2) where m1 at n1 and m2 at n2 in the mylist************)
...
(******************* (EQ b1 b2) where b1 at n1 and b2 at n2 in the mylist************)
...
(****************** b1 & b2 at given positions in the mylist**************************)
...
(****************** ~ b1 at given position in the mylist**************************)
...
(******************(If_then_else_M b1 m1 m2 : b1 at n1, m1 at n2, m2 at n3)****************)
...
(******************(If_then_else_B b1 b2 b3 : b1 at n1, b2 at n2, b3 at n3)****************)
...
(****************************pair term and a term at given pos ****************************)
...
(**************************************************************************************************************************************************************)
Scheme message_Bool_rec := Induction for message Sort Type
with Bool_message_rec := Induction for Bool Sort Type.
Check message_Bool_rec.
Check Bool_message_rec.
...
intros. 
Theorem  invarsub_Bmsg : forall (t:message) (n:nat), ((n:= (Bvar n))t = t).
Scheme message_Bool_mut := Induction for message Sort Prop
with Bool_message_mut := Induction for Bool Sort Prop.
Check message_Bool_mut.
apply (message_Bool_mut 
       ( fun t:message => forall n:nat, ((n:= (Bvar n)) t = t))
       (fun b:Bool => forall  n: nat, ( [n:= (Bvar n)] b = b))).
...
intros;simpl; rew_all_hyps;try reflexivity.
intros;simpl; rew_all_hyps;try reflexivity.
intros;simpl; rew_all_hyps;try reflexivity.
intros;simpl; rew_all_hyps;try reflexivity.
intros;simpl; rew_all_hyps;try reflexivity.
...
assert(forall l1 l2 :list message,  f l1 = f l2 ->  l1 = l2).
intros.
induction l1 ; induction l2.
reflexivity. simpl.  inversion H.
inversion H. 
inversion H. reflexivity.
inversion IHl.
rewrite H1. rewrite H1. simpl.
...
induction  n.   induction n0. reflexivity.
simpl. reflexivity.
induction n0. 
simpl. reflexivity.
