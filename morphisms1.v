...
Axiom bsign_cong: forall (m1 m1' m2 m2' : message), m1 # m1' -> m2 # m2' -> (bsign m1 m2) # (bsign m1' m2' ).
Add Parametric Morphism: (@bsign) with
  signature EQm ==> EQm ==> EQm as bsign_mor.
Proof.   intros. apply bsign_cong; assumption. Qed.
...
Axiom commit_cong: forall (m1 m2 m3 m1' m2' m3' : message), m1 # m1' -> m2 # m2' -> m3 # m3'  -> (commit m1 m2 m3) # (commit m1' m2' m3').
Add Parametric Morphism: (@ commit) with
    signature EQm ==> EQm ==> EQm ==> EQm as comit_mor.
Proof.   intros. apply commit_cong; assumption. Qed.

(** [opne_mor] *)

Axiom open_cong: forall (m1 m2 m3 m1' m2' m3' : message), m1 # m1' -> m2 # m2' -> m3 # m3'  -> (open m1 m2 m3) # (open m1' m2' m3').
Add Parametric Morphism: (@ open) with
  signature EQm ==> EQm ==> EQm ==> EQm as open_mor.
Proof.   intros. apply open_cong; assumption. Qed.

(** [blind_mor] *)

Axiom blind_cong: forall (m1 m2  m1' m2'  : message), m1 # m1' -> m2 # m2'  -> (blind m1 m2) # (blind m1' m2').
Add Parametric Morphism: (@ blind) with
  signature EQm ==> EQm ==> EQm as blind_mor.
Proof.   intros. apply blind_cong; assumption. Qed.

(** [ublind_mor] *)

Axiom unblind_cong: forall (m1 m2  m1' m2'  : message), m1 # m1' -> m2 # m2'  -> (unblind m1 m2) # (unblind m1' m2').
Add Parametric Morphism: (@ unblind) with
  signature EQm ==> EQm ==> EQm as ublind_mor.
 Proof.   intros. apply unblind_cong; assumption. Qed.

(** [v_mor] vote *)

Axiom v_cong: forall (m1 m1' : message), m1 # m1' -> (v m1) # (v m1' ).
Add Parametric Morphism: (@ v) with
  signature EQm ==> EQm as v_mor.
Proof. intros. apply v_cong. assumption. Qed.
(** [V_mor] Voter *)

Axiom V_cong: forall (n n' : nat), n =  n' -> (V n) # (V n' ).
Add Parametric Morphism: (@ V) with
  signature eq ==> EQm as V_mor.
Proof. intros. apply V_cong. reflexivity. Qed.
...
Axiom bver_cong : forall (m1 m2  m3 m1' m2' m3' : message), m1 # m1' -> m2 # m2' -> m3 # m3' -> (bver m1 m2 m3 ) ## (bver m1' m2' m3').
Add Parametric Morphism: (@bver) with
  signature EQm ==> EQm ==> EQm ==> EQb as bver_mor.
Proof.   intros. apply bver_cong; assumption. Qed.
(** [bacc_mor] *)

Axiom bacc_cong : forall (m1 m2  m3 m1' m2' m3' : message), m1 # m1' -> m2 # m2' -> m3 # m3' -> (bacc m1 m2 m3 ) ## (bacc m1' m2' m3').
Add Parametric Morphism: (@bacc) with
  signature EQm ==> EQm ==> EQm ==> EQb as bacc_mor.
Proof.   intros. apply bacc_cong; assumption. Qed.
